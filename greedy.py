...
def count_negations_in_kb(kb): # Used for the heuristic function.
    Dict = {}
    for clause in kb:
        temp_kb = kb[:]
        temp_kb.remove(clause)
        clause_string = str(clause)
        literals = clause_string.split('|')
        count = 0
        for literal in literals:
            string_of_kb = str(Or(*temp_kb))
            count += string_of_kb.count(str(eval('~'+literal)))
        Dict[clause] = count
...
if __name__ == "__main__":
    ...
    # Heuristic function counts the total number of negations of literal for each clause in the knowledge base.
    # The more the number of negations of a literal in the KB the more is the priority.
    negation_dict = count_negations_in_kb(kb)
    kb = sorted(kb, key=lambda clause: negation_dict[clause], reverse=True)

    # Heuristic function calculates the number of literals in an expression in the knowledge base.
    # If an expression has fewer literals, it will be chosen first for resolution.
    kb = sorted(kb, key=lambda formula: len(formula.args))
